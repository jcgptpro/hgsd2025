...
def load_default_logo():
    # 內建讀取 /mnt/data 的 Logo；若沒有，回傳 None
    p = "/mnt/data/logo_1_20220331026 (1).png"
    if os.path.exists(p):
        with open(p, "rb") as f:
            return f.read()
    return None
...
def try_load_persona_default():
    p = "/mnt/data/Persona_虛擬消費者_202508.xlsx"
    if os.path.exists(p):
        try:
...
def global_sidebar_nav():
    ...
    selected = st.sidebar.radio("功能選單", NAV, index=NAV.index(current), key="nav_selected")
    if selected != current:
...
UNIT_COST = {"FB_動態":0.6, "IG_限時":0.7, "Google_搜尋":1.0, "YouTube_展示":0.5}  # 千次成本 / 指標用
...
def estimate_by_mix(budget, mix):
    ...
    ctr = sum(UNIT_CTR.get(k,1.0)*w[k] for k in w) * 1.0
    cpa = sum(UNIT_CPA.get(k,150)*w[k] for k in w)
    cost = budget
    conv = int(max(cost/cpa, 0))
    roas = round((conv*300)/cost, 2) if cost>0 else 0   # 假設每轉換 300 TWD 價值
...
def page_login():
    ...
    with st.form("login"):
        ...
        with c1: sub = st.form_submit_button("登入")
        with c2: sso = st.form_submit_button("使用 SSO 登入")
        st.caption("忘記密碼")
        if sub or sso:
            if email.strip():
...
def m1_page():
    ...
    with tabs[0]:
        ...
        for i,k in enumerate(keys):
            mix[k] = cols[i].slider(k, 0, 100, int(mix[k]), 5)
        total = sum(mix.values())
        if total != 100:
            st.warning(f"目前合計：{total}%（建議調整為 100%）")
        st.session_state["channel_mix"] = mix

        est = estimate_by_mix(budget, mix)
    ...
    with tabs[1]:
        ...
        c1, c2 = st.columns(2)
        if c1.button("生成正式委刊單（含追蹤代碼）"):
            st.session_state["order_code"] = gen_order_code()
            st.success(f"已產生正式委刊單，追蹤代碼：{st.session_state['order_code']}")
        if c2.button("前往 TA 預測與圈選"):
    ...
    with tabs[2]:
        ...
        uploaded = st.file_uploader("上傳名單（CSV 或 XLSX）", type=["csv","xlsx"])
        if uploaded:
            try:
...
def normalize_persona(df):
    ...
    pcm_col  = pick(["PCM","情感動機","情緒"], None)
    vcm_col  = pick(["VCM","功能動機","理性"], None)
    ...
    for _,row in df.iterrows():
        ...
        pcm  = str(row.get(pcm_col,"情感訴求"))
        vcm  = str(row.get(vcm_col,"功能利益"))
        items.append({
            "name": name, "size": size, "pain": pain, "keywords": kw,
            "slots": slot, "attitudes": att, "PCM": pcm, "VCM": vcm
        })
    return items
...
def m2_page():
    ...
    st.write("Persona 來源：")
    uploaded = st.file_uploader("上傳 Persona Excel（若未自動載入）", type=["xlsx"])
    if uploaded:
        df = pd.read_excel(uploaded, engine="openpyxl")
    ...
    for idx, it in enumerate(items):
        with cols[idx % 3]:
            ...
            checked = st.checkbox("選擇此 TA", key=f"ta_{idx}", value=(it['name'] in selections))
            if checked:
...
def m3_page():
    ...
    with tabs[0]:
        ...
        for ta in ta_list:
            ...
            arr = [f"{random.choice(hooks)}！{ta}的你，{random.choice(pains)}？{random.choice(ctas)}。" for _ in range(5)]
            copy_map[ta] = arr
        ...
        frame_type = st.selectbox("選擇圖片版型", ["A 情境寫實","B 產品特寫","C 前後對比"])
        prompt_all = []
        for ta in ta_list:
            for ch in (channels or ["FB_動態"]):
                prompt_all.append(build_image_prompt(ta, frame_type, ch))
        if prompt_all:
            st.code("\n\n".join(prompt_all), language="text")
            st.download_button("下載 prompts.txt", data=("\n\n".join(prompt_all)).encode("utf-8"), file_name="prompts.txt")
            if st.button("請 AI 生圖（示意）"):
                img_bytes = generate_placeholder_image(" ".join(prompt_all[:3]))
                st.image(img_bytes, caption="AI 生成占位圖（Demo）")
                st.download_button("下載生成圖片", data=img_bytes, file_name="ai_image.png")
        else:
...
def m4_page():
    ...
    with tabs[0]:
        ...
        c1, c2 = st.columns(2)
        if c1.button("加入再行銷提案"):
            st.session_state["remarketing_tag"] = True
            st.session_state["current_page"] = "📝 提案目標與報價"
            set_query_page("📝 提案目標與報價")
            st.rerun()
        if c2.button("下載週報 CSV"):
            buff = io.StringIO()
    ...
    with tabs[1]:
        ...
        st.dataframe(codes)
        st.download_button("下載 open_coding.csv", data=codes.to_csv(index=False), file_name="open_coding.csv")
...
def m7_page():
    ...
    st.subheader("方案與使用量")
    plan = st.selectbox("目前方案", ["Free","Team","Professional","Enterprise"], index=0)
    usage = pd.DataFrame({"項目":["AI 對話","市調回收","生成素材"], "本月用量":[42, 186, 12], "額度":[100, 1000, 100]})
    st.dataframe(usage)

    st.subheader("付款方式")
    with st.form("pay"):
        name = st.text_input("持卡人")
        card = st.text_input("卡號（只示意）", value="**** **** **** 4242")
        exp = st.text_input("到期", value="12/29")
    ...
    with st.form("invoice"):
        title = st.text_input("抬頭")
        taxid = st.text_input("統編")
        addr = st.text_area("地址", height=80)
...
def m8_page():
    ...
    with col1:
        up = st.file_uploader("上傳 Logo（PNG）", type=["png"])
        if up:
    ...
    st.data_editor(members, num_rows="dynamic")

    st.subheader("安全設定")
    st.toggle("啟用 2FA", value=False)
    st.text_input("密碼策略（示意）", value="至少 12 碼、含大小寫與數字")

    st.subheader("API 金鑰")
    c1,c2 = st.columns(2)
    with c1:
        st.text_input("OpenAI API Key（示意，不儲存）", type="password")
    with c2:
        if st.button("重置 API 金鑰"):
            st.info("已重置（示意）")
