...
# ===== Tracking Code Store Helpers (non-invasive) =====
def gen_order_code_kind(kind):
    return gen_order_code(kind)

def ensure_code_store():
    ss = st.session_state
    if "codes" not in ss:
        ss["codes"] = {}
        # ---- Seed demo codes ----
        # A 客戶：兩個媒體案（未鎖定/未確定、已鎖定/已確定）
        ss["codes"]["HG-MED-2025-08-0001"] = {
            "code":"HG-MED-2025-08-0001","kind":"MED","client":"A 客戶","goal":"曝光","industry":"美妝",
            "budget":600000,"start":date(2025,8,15),"end":date(2025,8,31),
            "channel_mix":{"FB":25,"Google":25,"Line":10,"SMS":5,"EDM":10,"APP廣告":15,"APP任務":5,"APP Push":5},
            "ta_locked": False,"personas_selected":[],"creatives_finalized": False,
            "performance":{"CTR":1.2,"CPA":160,"Spend":420000,"Impr":1200000,"Clicks":14400,"Conv":2625},
            "created_at":"2025-08-10"
        }
        ss["codes"]["HG-MED-2025-08-0002"] = {
            "code":"HG-MED-2025-08-0002","kind":"MED","client":"A 客戶","goal":"購買","industry":"家電",
            "budget":1200000,"start":date(2025,8,1),"end":date(2025,8,28),
            "channel_mix":{"Google":35,"FB":20,"EDM":15,"Line":10,"APP Push":8,"SMS":5,"APP廣告":5,"APP任務":2},
            "ta_locked": True,"personas_selected":["理性比價族","科技控人士"],"creatives_finalized": True,
            "performance":{"CTR":2.4,"CPA":110,"Spend":980000,"Impr":2200000,"Clicks":52800,"Conv":8909},
            "created_at":"2025-08-03"
        }
        # B 客戶：兩個市調案（高回收、低回收）
        ss["codes"]["HG-SUY-2025-08-0001"] = {
            "code":"HG-SUY-2025-08-0001","kind":"SUY","client":"B 客戶","goal":"意向調查","industry":"FMCG",
            "budget":250000,"start":date(2025,8,5),"end":date(2025,8,20),
            "ta_locked": False,"personas_selected":[],
            "survey":{"target":"全台 18-45 歲","qtype":"混合","n":1200,"closed_stats":{"滿意":0.62,"中立":0.25,"不滿":0.13},
                      "open_coding":[{"code":"包裝設計好看","count":124},{"code":"口味偏甜","count":78},{"code":"價格稍高","count":55}]},
            "created_at":"2025-08-06"
        }
        ss["codes"]["HG-SUY-2025-08-0002"] = {
            "code":"HG-SUY-2025-08-0002","kind":"SUY","client":"B 客戶","goal":"新產品測試","industry":"保健",
            "budget":180000,"start":date(2025,8,12),"end":date(2025,8,25),
            "ta_locked": False,"personas_selected":[],
            "survey":{"target":"女性 25-45","qtype":"封閉型","n":280,"closed_stats":{"願意嘗試":0.38,"觀望":0.42,"不考慮":0.20},
                      "open_coding":[{"code":"效期擔憂","count":22},{"code":"看不懂成分","count":17}]},
            "created_at":"2025-08-13"
        }
    if "serial_med" not in ss: ss["serial_med"] = {}
    if "serial_suy" not in ss: ss["serial_suy"] = {}

def _next_serial(ym, bucket):
    ss = st.session_state
    d = ss[bucket]
    d[ym] = d.get(ym, 0) + 1
    return d[ym]

def gen_order_code(kind):
    # kind: "MED" 媒體；"SUY" 市調
    now = datetime.now()
    ym = now.strftime("%Y-%m")
    if kind == "MED":
        seq = _next_serial(ym, "serial_med")
        return f"HG-MED-{now.strftime('%Y-%m')}{seq:04d}"
    else:
        seq = _next_serial(ym, "serial_suy")
        return f"HG-SUY-{now.strftime('%Y-%m')}{seq:04d}"

def list_codes(kind=None, client=None):
    ensure_code_store()
    codes = st.session_state["codes"]
    arr = [c for c in codes.values() if (kind is None or c["kind"]==kind) and (client is None or c.get("client")==client)]
    arr.sort(key=lambda x: (x["kind"], x["created_at"], x["code"]))
    return arr

def get_code(code):
    ensure_code_store()
    return st.session_state["codes"].get(code)

def upsert_code(obj):
    ensure_code_store()
    st.session_state["codes"][obj["code"]] = obj

def snapshot_from_m11(code):
    obj = get_code(code) or {"code":code,"kind":"MED"}
    obj.update({
        "client": st.session_state.get("m11_brand","未命名客戶"),
        "goal": st.session_state.get("m11_goal","曝光"),
        "industry": st.session_state.get("m11_industry","其他"),
        "budget": st.session_state.get("m11_budget",0),
        "start": st.session_state.get("m11_start", date.today()),
        "end": st.session_state.get("m11_end", date.today()),
        "channel_mix": st.session_state.get("channel_mix", {}),
        "ta_locked": obj.get("ta_locked", False),
        "personas_selected": obj.get("personas_selected", []),
        "creatives_finalized": obj.get("creatives_finalized", False),
        "performance": obj.get("performance", {"CTR":1.5,"CPA":150,"Spend":300000,"Impr":1000000,"Clicks":15000,"Conv":2000}),
        "created_at": obj.get("created_at", datetime.now().strftime("%Y-%m-%d"))
    })
    upsert_code(obj)

def snapshot_from_m12(code):
    obj = get_code(code) or {"code":code,"kind":"SUY"}
    obj.update({
        "client": st.session_state.get("m11_brand","未命名客戶"),
        "goal": st.session_state.get("m12_target","一般調查"),
        "industry": st.session_state.get("m11_industry","其他"),
        "budget": obj.get("budget", 150000),
        "start": obj.get("start", date.today()),
        "end": obj.get("end", date.today()+timedelta(days=7)),
        "ta_locked": obj.get("ta_locked", False),
        "personas_selected": obj.get("personas_selected", []),
        "survey": obj.get("survey", {"target": st.session_state.get("m12_target",""), "qtype": st.session_state.get("m12_qtype","封閉型"), "n": st.session_state.get("m12_n",300)}),
        "created_at": obj.get("created_at", datetime.now().strftime("%Y-%m-%d"))
    })
    upsert_code(obj)
# ===== End Tracking Code Store Helpers =====
...
def gen_order_code():
    # legacy function kept for compatibility, default to MED
    return gen_order_code_kind("MED")
...
def quote_by_days_and_mix(days, mix, day_rate=None):
    if day_rate is None:
        day_rate = DAY_RATE
    if not mix or sum(mix.values()) == 0:
        return 0, {}
...
def m1_page():
    ...
    with tabs[0]:
        ensure_code_store()
        # 追蹤代碼下拉（此客戶歷史 + 新增提案）
        client_name = st.session_state.get("m11_brand","")
        med_codes = ["新增提案"] + [c["code"] for c in list_codes(kind="MED", client=client_name) or list_codes(kind="MED")]
        sel_med = st.selectbox("選擇追蹤代碼（媒體）", med_codes, index=0, key="m11_code_selector")
        readonly = False
        if sel_med != "新增提案":
            st.session_state["order_code"] = sel_med
            data = get_code(sel_med)
            # 將值填入並鎖定
            st.session_state["m11_brand"] = data.get("client","")
            st.session_state["m11_industry"] = data.get("industry","其他")
            st.session_state["m11_goal"] = data.get("goal","曝光")
            st.session_state["m11_budget"] = data.get("budget",0)
            st.session_state["m11_start"] = data.get("start", date.today())
            st.session_state["m11_end"] = data.get("end", date.today())
            st.session_state["channel_mix"] = data.get("channel_mix",{})
            readonly = True
            st.info("正在檢視歷史提案（唯讀）。若要新增，請選擇『新增提案』。")
        st.subheader("蒐集需求")
        with st.expander("輸入引導", expanded=True):
            col1, col2 = st.columns(2)
            brand = col1.text_input("品牌名稱", key="m11_brand", disabled=readonly)
            industry = col2.selectbox("產業", ["保健","運動/健身","寵物","家電","FMCG","美妝","其他"], key="m11_industry", disabled=readonly)
            goal = col1.selectbox("行銷目標", ["曝光","名單","購買"], key="m11_goal", disabled=readonly)
            budget = col2.number_input("預算（TWD）", min_value=0, step=10000, value=200000, key="m11_budget", disabled=readonly)
            start_d = col1.date_input("檔期（開始）", value=date.today(), key="m11_start", disabled=readonly)
            end_d = col2.date_input("檔期（結束）", value=date.today() + timedelta(days=13), key="m11_end", disabled=readonly)
        ...
        if st.session_state.get("show_ai_m11"):
            ...
            with st.container():
                ...
                if prompt:
                    ...
                    ai = f"已根據你的目標「{st.session_state.get('m11_goal','')}」與產業「{st.session_state.get('m11_industry','')}」套用 8 渠道推薦配比（可再微調）。系統會連動預估報價與成效。"
                    st.session_state["chat_m11"].append(("assistant", ai))
                    with st.chat_message("assistant"):
                        st.markdown(ai)
                    st.session_state["channel_mix"] = GOAL_TEMPLATES.get(st.session_state.get("m11_goal","曝光"), GOAL_TEMPLATES["曝光"]).copy()
        ...
        for idx, ch in enumerate(CHANNELS_8):
            mix[ch] = cols[idx % 4].slider(ch, 0, 100, int(mix.get(ch, 0)), 1, key=f"m11_mix_{ch}", disabled=readonly)
        ...
        st.caption("此配比將同步帶入『渠道與文案製作』頁面。")
        ...
        if st.button("生成正式委刊單（含追蹤代碼）", key="m11_gen_io"):
            code = gen_order_code("MED")
            st.session_state["order_code"] = code
            snapshot_from_m11(code)
            st.success(f"已產生正式委刊單，追蹤代碼：{code}")
            st.session_state["m11_code_selector"] = code
            # 初始化 M3 渠道模板（依目標）
    ...
    with tabs[1]:
        ...
        if st.button("生成正式委刊單（含追蹤代碼）", key="m12_gen_io"):
            code = gen_order_code("SUY")
            st.session_state["order_code"] = code
            snapshot_from_m12(code)
            st.success(f"已產生正式委刊單，追蹤代碼：{code}")
            st.session_state["m12_code_selector"] = code
...
def normalize_persona(df):
    ...
    att_col  = pick(["消費態度","態度","傾向","Attitude"], None)

    import numpy as _np
    rng = _np.random.default_rng(2025)
    pain_pool = ["價格敏感","怕踩雷","資訊過載","選擇困難","時間不足","需要快速見效","售後疑慮","物流/取貨不便","品牌信任不足","比較成本高"]
    att_pool = ["品牌忠誠高","品牌忠誠中等","品牌忠誠低","重口碑","重規格","重CP值","重體驗","嘗鮮型","保守型","線上偏好","線下偏好","混合通路"]

    items = []
    used_sizes = set()
    for idx, (_, row) in enumerate(df.iterrows()):
        name = str(row.get(name_col, f"Persona_{idx+1}"))
        size = int(rng.integers(50000, 300001))
        tries = 0
        while size in used_sizes and tries < 5:
            size = int(rng.integers(50000, 300001)); tries += 1
        used_sizes.add(size)

        raw_pain = row.get(pain_col, None)
        if raw_pain is None or (isinstance(raw_pain, float) and _np.isnan(raw_pain)) or str(raw_pain).strip()=="" or idx < len(pain_pool):
            pain = pain_pool[idx % len(pain_pool)]
        else:
            pain = str(raw_pain)

        kw   = str(row.get(kw_col,"")) if kw_col else ""
        slot = str(row.get(slot_col,"")) if slot_col else "FB/Google/Line"

        raw_att = row.get(att_col, None) if att_col else None
        if raw_att is None or (isinstance(raw_att, float) and _np.isnan(raw_att)) or str(raw_att).strip()=="":
            atts = rng.choice(att_pool, size=2, replace=False)
            attitudes = "、".join(atts.tolist())
        else:
            attitudes = str(raw_att)
...
def m2_page():
    ...
    if not all_codes:
        st.info("目前沒有任何追蹤代碼，請先前往「提案目標與報價」生成正式委刊單。")
        return
    code_opt = st.selectbox("選擇追蹤代碼", [c["code"] for c in all_codes], key="m2_code_sel")
    st.session_state["order_code"] = code_opt
    _code_obj = get_code(code_opt)
    _locked = _code_obj.get("ta_locked", False)
    ...
    for i, it in enumerate(recs):
        with cols[i % 5]:
            ...
            st.caption(f"痛點：{it['pain']}｜消費態度：{it['attitudes']}")
...
def m3_page():
    ...
    with tabs[0]:
        ensure_code_store()
        med_codes = [c["code"] for c in list_codes(kind="MED")]
        if not med_codes:
            st.info("目前沒有媒體類追蹤代碼，請先至提案產生。")
        else:
            code_sel = st.selectbox("選擇追蹤代碼（媒體）", med_codes, key="m31_code_sel")
            _obj = get_code(code_sel)
            st.subheader("選擇渠道與版位（依提案目標預設配比，可調整）")
            if _obj.get("creatives_finalized", False):
                st.warning("此追蹤代碼已完成『確定』，不可再調整。")
            # 權重來源：優先取代碼的 channel_mix，其次取 1-1 的暫存，最後套目標模板
            weights = _obj.get("channel_mix").copy() if _obj.get("channel_mix") else (st.session_state.get("channel_mix").copy() if st.session_state.get("channel_mix") else channel_defaults_from_goal().copy())
            ...
            for idx, ch in enumerate(CHANNELS_8):
                if ch in selected:
                    with cols[idx % 4]:
                        new_weights[ch] = st.number_input(ch, min_value=0, max_value=100, value=int(weights.get(ch,0)), step=1, key=f"m31_w_{ch}", disabled=_obj.get("creatives_finalized", False))
            total = sum(new_weights.values()) if new_weights else 0
            if total != 100 and total != 0:
                st.warning(f"目前合計：{total}%（建議調整為 100%）")
            st.session_state["m31_weights"] = new_weights
            if new_weights:
                _obj["channel_mix"] = new_weights
                upsert_code(_obj)
            # 確定鎖定
            if not _obj.get("creatives_finalized", False):
                if st.button("✅ 確定（鎖定渠道與文案設定）", key="m31_confirm"):
                    _obj["creatives_finalized"] = True
                    upsert_code(_obj)
                    st.success("已確定，之後不可再調整。")
        st.divider()
