...
def inject_sidebar_css():
    st.markdown(
        """
        <style>
        /* Sidebar nav buttons: no border, transparent, tighter spacing */
...
# ---------- Module 2：TA 預測與圈選（AI 推薦 5 個，展開更多） ----------
...
def m4_page():
    ...
    with tabs[0]:
        ...
        st.write("- 亮點：產品特寫（B）在 Google CTR 最高；FB 對年輕族群表現佳。")
        st.write("- 風險：FB 出現疲勞（頻率過高、CPC 上升）。")
        st.write("- 建議：提高 Google 10% 預算；FB 更換文案鉤子；嘗試 APP Push 提升回訪。")
        c1, c2 = st.columns(2)
