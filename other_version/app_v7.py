...
def m1_page():
    ...
    with tabs[0]:
        ...
        for idx, ch in enumerate(CHANNELS_8):
            mix[ch] = cols[idx % 4].slider(ch, 0, 100, int(mix.get(ch, 0)), 1, key=f"m11_mix_{ch}")
...
def m4_page():
    ...
    with tabs[1]:
        ...
        pie = alt.Chart(_gdf).mark_arc().encode(
            theta=alt.Theta(field="人數", type="quantitative"),
            color=alt.Color(field="性別", type="nominal"),
            tooltip=["性別","人數"]
        )
        ...
        try:
            ...
            pass
        _adf = _adf.reset_index(drop=True)
        bar = alt.Chart(_adf).mark_bar().encode(
            y=alt.Y("年齡層:N", sort='-x'),
            x=alt.X("人數:Q"),
            tooltip=["年齡層","人數"]
        )
