...
def nav_button(label, active=False, key=None):
    ...
    if st.sidebar.button(display, key=key, use_container_width=True, type="secondary" if active else "primary"):
        if not active:
...
def m4_page():
    ...
    with tabs[0]:
        ...
        st.subheader("圖片版型 × 文案 × 版位 矩陣（示意）")
        df = matrix_sample()
        st.dataframe(df)
        st.subheader("AI 洞察（示意）")
        st.write("- 亮點：產品特寫（B）在 Google CTR 最高；Line/SMS 對回流轉換有幫助。")
